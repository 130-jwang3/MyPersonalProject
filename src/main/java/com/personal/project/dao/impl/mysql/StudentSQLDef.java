...
public class StudentSQLDef {
    ...
    public static final String STU_FIND_BY_NAME = " SELECT * FROM student join person on person_oid=person.oid join (select student_oid,avg(course_grade) num_grade from course_student_map group by student_oid) csmap on student_oid=student.oid where first_name like ? or last_name like ? group by student_id ";
    public static final String STU_DELETE_BY_ID = " delete from student where student_id=(?) ";
    public static final String STU_UPDATE = " update student,person set last_name=?,first_name=?, phone=?,major=? where person_oid=person.oid and student_id=? ";
